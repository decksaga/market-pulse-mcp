...
// ─── TOOL: get_forex_rate ───
server.tool("get_forex_rate", "Get the current exchange rate between two currencies (forex). Examples: USD to EUR, GBP to JPY.", {
...
server.tool("get_top_cryptos", "Get the top cryptocurrencies by market cap with prices and 24h changes.", { limit: z.number().min(1).max(25).default(10).describe("Number of cryptos to return (1-25, default 10)") }, async ({ limit }) => {
    try {
...
server.tool("get_fear_greed_index", "Get the Crypto Fear & Greed Index. Values: 0-25 = Extreme Fear, 25-50 = Fear, 50-75 = Greed, 75-100 = Extreme Greed.", {}, async () => {
    try {
        const data = await getFearGreedIndex();
        const { current, history } = data;
        const emoji = current.value <= 25 ? "😱" :
            current.value <= 50 ? "😰" :
                current.value <= 75 ? "😀" : "🤑";
        const historyLines = history.slice(1).map(h => `  ${h.date}: ${h.value} (${h.classification})`);
        const text = [
...
server.tool("get_market_summary", "Get a complete market overview: top 5 cryptos, major forex rates, and the Fear & Greed Index. Great for a quick daily briefing.", {}, async () => {
    try {
        ...
        const forexLines = data.forex.map(f => `  ${f.pair}: ${f.rate?.toFixed(4) ?? "N/A"}`);
